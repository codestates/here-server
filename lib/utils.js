...
module.exports = {
	...
	tokenCheck: async (req, res, next) => {
		...
		if (typeof authData === "undefined") {
			let checkoutRefreshToken = await jwt.verify(
				req.refreshToken,
				process.env.JWT_PUBLIC,
				(err, authData) => (err ? res.send("리프레시 토큰도 만료") : authData),
			);

			const tempObj = { ...checkoutRefreshToken };

			let result = jwt.sign({ id: tempObj.id }, process.env.JWT_PUBLIC, {
				expiresIn: "5s",
			});
